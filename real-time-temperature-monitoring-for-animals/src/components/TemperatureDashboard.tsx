...
export default function TemperatureDashboard() {
  const { animalId } = useParams() as { animalId: string }; // Get animalId from URL
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      // Log the exact URL being used for the fetch request
      console.log(`Fetching data from: http://127.0.0.1:8000/api/temperatures/${animalId}`);
  ...
  useEffect(() => {
    ...
    const alertRes = fetch("http://127.0.0.1:8000/api/alerts");
    alertRes
